...
HRESULT LoadD3DCompiler()
{
	...
	d3d_compiler_dll = LoadLibraryA(D3DCOMPILER_DLL_A);
	if (!d3d_compiler_dll)
	{
		// if that fails, use the dll which should be available in every SDK which officially supports DX12.
		d3d_compiler_dll = LoadLibraryA("D3DCompiler_42.dll");
		if (!d3d_compiler_dll)
		{
	...
	d3d_reflect = (D3DREFLECT) GetProcAddress(d3d_compiler_dll, "D3DReflect");
	if (d3d_reflect == nullptr)
		MessageBoxA(nullptr, "GetProcAddress failed for D3DReflect!", "Critical error", MB_OK | MB_ICONERROR);

	d3d_compile = (pD3DCompile) GetProcAddress(d3d_compiler_dll, "D3DCompile");
...
void UnloadD3DCompiler()
{
	...
	if (d3d_compiler_dll)
		FreeLibrary(d3d_compiler_dll);

	d3d_compiler_dll = nullptr;
	d3d_reflect = nullptr;
}
