...
static void TeardownDeviceObjects()
{
	...
	{
		D3D::commandListMgr->DestroyResourceAfterCurrentCommandListExecuted(s_screenshot_texture);
		s_screenshot_texture = nullptr;
	}

	D3D::commandListMgr->DestroyResourceAfterCurrentCommandListExecuted(access_efb_cbuf12);
...
Renderer::Renderer(void *&window_handle)
{
	...
	D3D::currentCommandList->ClearRenderTargetView(FramebufferManager::GetEFBColorTexture()->GetRTV12(), ClearColor, 0, nullptr);
	D3D::currentCommandList->ClearDepthStencilView(FramebufferManager::GetEFBDepthTexture()->GetDSV12(), D3D12_CLEAR_FLAG_DEPTH, 0.f, 0, 0, nullptr);
...
EFBRectangle currentRectangle = {};

void Renderer::SetScissorRect(const EFBRectangle& rc)
{
	TargetRectangle trc = ConvertEFBRectangle(rc);
	D3D::currentCommandList->RSSetScissorRects(1, trc.AsRECT());

	currentRectangle = rc;
}

EFBRectangle Renderer::GetScissorRect()
{
...
u32 Renderer::AccessEFB(EFBAccessType type, u32 x, u32 y, u32 poke_data)
{
	...
	if (type == PEEK_Z)
	{
		...
		D3D::commandListMgr->CPUAccessNotify();
#endif
		...
		D3D::currentCommandList->SetGraphicsRootConstantBufferView(DESCRIPTOR_TABLE_PS_CBVONE, access_efb_cbuf12->GetGPUVirtualAddress());
		D3D::commandListMgr->dirtyPSCBV = true;

		FramebufferManager::GetEFBDepthReadTexture()->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_RENDER_TARGET);
		D3D::currentCommandList->OMSetRenderTargets(1, &FramebufferManager::GetEFBDepthReadTexture()->GetRTV12(), FALSE, nullptr);
		...
		FramebufferManager::GetEFBDepthReadTexture()->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_COPY_SOURCE);
		D3D::currentCommandList->CopyTextureRegion(&dstLocation, 0, 0, 0, &srcLocation, &box12);
	...
	else if (type == PEEK_COLOR)
	{
		...
		FramebufferManager::GetEFBColorTexture()->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_COPY_SOURCE);
		D3D::currentCommandList->CopyTextureRegion(&dstLocation, 0, 0, 0, &srcLocation, &box12);
	...
	else if(type == POKE_COLOR)
	{
		...
		FramebufferManager::GetEFBColorTexture()->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_RENDER_TARGET);
		D3D::currentCommandList->OMSetRenderTargets(1, &FramebufferManager::GetEFBColorTexture()->GetRTV12(), FALSE, nullptr);
...
{
	D3D12_BLEND_DESC *pBlendDesc = nullptr;

	if (colorEnable && alphaEnable) pBlendDesc = &clearblendstates12[0];
	else if (colorEnable) pBlendDesc = &clearblendstates12[1];
	else if (alphaEnable) pBlendDesc = &clearblendstates12[2];
	else pBlendDesc = &clearblendstates12[3];

	D3D12_DEPTH_STENCIL_DESC *pDepthStencilDesc = nullptr;

	// TODO: Should we enable Z testing here?
	/*if (!bpmem.zmode.testenable) pDepthStencilDesc = &cleardepthstates[0];
	else */if (zEnable) pDepthStencilDesc = &cleardepthstates12[1];
	else /*if (!zEnable)*/ pDepthStencilDesc = &cleardepthstates12[2];
...
void Renderer::ReinterpretPixelData(unsigned int convtype)
{
	...
	FramebufferManager::GetEFBColorTempTexture()->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_RENDER_TARGET);
	D3D::currentCommandList->OMSetRenderTargets(1, &FramebufferManager::GetEFBColorTempTexture()->GetRTV12(), FALSE, nullptr);
...
bool Renderer::SaveScreenshot(const std::string &filename, const TargetRectangle& rc)
{
	...
	D3D::GetBackBuffer()->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_COPY_SOURCE);
	D3D::currentCommandList->CopyTextureRegion(&dst, 0, 0, 0, &src, &source_box);
...
void Renderer::SwapImpl(u32 xfbAddr, u32 fbWidth, u32 fbStride, u32 fbHeight, const EFBRectangle& rc, float Gamma)
{
	...
	D3D::GetBackBuffer()->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_RENDER_TARGET);
	D3D::currentCommandList->OMSetRenderTargets(1, &D3D::GetBackBuffer()->GetRTV12(), FALSE, nullptr);

	float ClearColor[4] = { 0.f, 0.f, 0.f, 1.f };
	D3D::currentCommandList->ClearRenderTargetView(D3D::GetBackBuffer()->GetRTV12(), ClearColor, 0, nullptr);
	...
	D3D::currentCommandList->RSSetScissorRects(1, &backBufferRect);
		...
		s_last_stereo_mode != (g_ActiveConfig.iStereoMode > 0))
	{
		...
		FramebufferManager::GetEFBColorTexture()->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_RENDER_TARGET);
		D3D::currentCommandList->ClearRenderTargetView(FramebufferManager::GetEFBColorTexture()->GetRTV12(), clear_col, 0, nullptr);
...
void Renderer::ApplyState(bool bUseDstAlpha)
{
	...
	if (D3D::commandListMgr->dirtySamplers)
	{
		D3D12_GPU_DESCRIPTOR_HANDLE samplerGroupGpuHandle;
		samplerGroupGpuHandle = D3D::samplerDescriptorHeapMgr->GetHandleForSamplerGroup(gx_state.sampler, 8);

		D3D::currentCommandList->SetGraphicsRootDescriptorTable(DESCRIPTOR_TABLE_PS_SAMPLER, samplerGroupGpuHandle);

		D3D::commandListMgr->dirtySamplers = false;
	}
	...
	if (D3D::commandListMgr->dirtyPso || pOldVertextFormat != reinterpret_cast<D3DVertexFormat*>(VertexLoaderManager::GetCurrentVertexFormat()))
	{
		...
		CheckHR(gx_state_cache.GetPipelineStateObjectFromCache(&psoDesc, &pPso, topologyType, PixelShaderCache::GetActiveShaderUid12(), VertexShaderCache::GetActiveShaderUid12(), GeometryShaderCache::GetActiveShaderUid12()));

		D3D::currentCommandList->SetPipelineState(pPso);

		D3D::commandListMgr->dirtyPso = false;
	}
}
...
void Renderer::BlitScreen(TargetRectangle src, TargetRectangle dst, D3DTexture2D* src_texture, u32 src_width, u32 src_height, float Gamma)
{
	...
	else if (g_ActiveConfig.iStereoMode == STEREO_3DVISION)
	{
		...
		s_3d_vision_texture->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_RENDER_TARGET);
		...
		D3D::GetBackBuffer()->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_COPY_DEST);
		s_3d_vision_texture->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_COPY_SOURCE);
		D3D::currentCommandList->CopyTextureRegion(&dst, 0, 0, 0, &src, &box);
