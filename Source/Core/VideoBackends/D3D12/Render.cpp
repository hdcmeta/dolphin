...
u32 Renderer::AccessEFB(EFBAccessType type, u32 x, u32 y, u32 poke_data)
{
	...
	if (type == PEEK_Z)
	{
		...
		float val = 1.0f - *((float*)((u8*)readback_buffer_data + D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT));
		u32 ret = 0;
	...
	else if (type == PEEK_COLOR)
	{
		...
		u32 ret = *(u32*)((u8*)readback_buffer_data + D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT);
	...
	else if(type == POKE_COLOR)
	{
		...
		D3D::DrawColorQuad(rgbaColor, 0.f,
		 	  (float)rect_to_lock.left   * 2.f / GetTargetWidth() - 1.f,
			- (float)rect_to_lock.top    * 2.f / GetTargetHeight() + 1.f,
			  (float)rect_to_lock.right  * 2.f / GetTargetWidth() - 1.f,
			- (float)rect_to_lock.bottom * 2.f / GetTargetHeight() + 1.f,
			&g_reset_blend_desc,
			&g_reset_depth_desc,
	...
	{
		// EXISTINGD3D11TODO: The first five PE registers may change behavior of EFB pokes, this isn't implemented, yet.
...
bool Renderer::SaveScreenshot(const std::string& filename, const TargetRectangle& rc)
{
	...
	saved_png = TextureToPng((u8*)s_screenshot_texture_data + D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT, dst_location.PlacedFootprint.Footprint.RowPitch, filename, source_box.right - source_box.left, source_box.bottom - source_box.top, false);
...
void Renderer::SwapImpl(u32 xfb_addr, u32 fb_width, u32 fb_stride, u32 fb_height, const EFBRectangle& rc, float gamma)
{
	...
	else if (g_ActiveConfig.bUseXFB)
	{
		...
		for (u32 i = 0; i < xfb_count; ++i)
		{
			...
			source_rc.left = 0;
			source_rc.top = 0;
			source_rc.right = (int)xfb_source->texWidth;
			source_rc.bottom = (int)xfb_source->texHeight;
	...
	if (SConfig::GetInstance().m_DumpFrames)
	{
		...
		if (bAVIDumping)
		{
			...
			formatBufferDump((u8*)s_screenshot_texture_data + D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT, &frame_data[0], source_width, source_height, dst_location.PlacedFootprint.Footprint.RowPitch);
