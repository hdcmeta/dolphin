...
void TextureCache::TCacheEntry::Bind(unsigned int stage, unsigned int lastTexture)
{
	if (lastTexture == 0)
	{
		DX12::D3D::currentCommandList->SetGraphicsRootDescriptorTable(DESCRIPTOR_TABLE_PS_SRV, this->srvGpuHandle);
		return;
	}
	...
	if (firstTextureInGroup)
	{
		...
		DX12::D3D::gpuDescriptorHeapMgr->AllocateGroup(&groupBaseTextureCpuHandle, 8, &groupBaseTextureGpuHandle, nullptr, true);

		// Pave over space with null textures.
		for (unsigned int i = 0; i < lastTexture; i++)
		{
			D3D12_CPU_DESCRIPTOR_HANDLE nullDestDescriptor;
			nullDestDescriptor.ptr = groupBaseTextureCpuHandle.ptr + i * D3D::resourceDescriptorSize;

			DX12::D3D::device12->CopyDescriptorsSimple(
				1,
				nullDestDescriptor,
				DX12::D3D::nullSRVCPUshadow,
	...
	textureDestDescriptor.ptr = groupBaseTextureCpuHandle.ptr + stage * D3D::resourceDescriptorSize;
	DX12::D3D::device12->CopyDescriptorsSimple(
		1,
	...
	if (stage == lastTexture)
	{
		...
		DX12::D3D::currentCommandList->SetGraphicsRootDescriptorTable(DESCRIPTOR_TABLE_PS_SRV, groupBaseTextureGpuHandle);
...
bool TextureCache::TCacheEntry::Save(const std::string& filename, unsigned int level)
{
	...
	texture->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_COPY_SOURCE);
	...
	D3D::currentCommandList->CopyTextureRegion(&dst, 0, 0, 0, &src, nullptr);
	...
	const MathUtil::Rectangle<int> &dstrect)
{
		...
		&& srcrect.GetHeight() == dstrect.GetHeight())
	{
		...
		texture->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_COPY_DEST);
		srcentry->texture->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_COPY_SOURCE);

		D3D::currentCommandList->CopyTextureRegion(&dst, dstrect.left, dstrect.top, 0, &src, psrcbox);
	...
	texture->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_RENDER_TARGET);
	D3D::currentCommandList->OMSetRenderTargets(1, &texture->GetRTV12(), FALSE, nullptr);
	...
	bool scaleByHalf, unsigned int cbufid, const float *colmat)
{
	...
	D3D::currentCommandList->SetGraphicsRootConstantBufferView(DESCRIPTOR_TABLE_PS_CBVONE, efbcopycbuf12[cbufid]->GetGPUVirtualAddress());
...
void TextureCache::ConvertTexture(TCacheEntryBase* entry, TCacheEntryBase* unconverted, void* palette, TlutFormat format)
{
	...
	D3D::gpuDescriptorHeapMgr->AllocateGroup(srvGroupCpu, 2, srvGroupGpu, nullptr, true);

	srvGroupCpu[1].ptr = srvGroupCpu[0].ptr + D3D::resourceDescriptorSize;
	...
	static_cast<TCacheEntry*>(unconverted)->texture->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
	...
	D3D::currentCommandList->SetGraphicsRootDescriptorTable(DESCRIPTOR_TABLE_PS_SRV, srvGroupGpu[0]);
	...
	{
		D3D::currentCommandList->SetGraphicsRootConstantBufferView(DESCRIPTOR_TABLE_PS_CBVONE, palette_uniform12->GetGPUVirtualAddress());
	}
	else
	{
		D3D::currentCommandList->SetGraphicsRootConstantBufferView(DESCRIPTOR_TABLE_PS_CBVONE, palette_uniform12->GetGPUVirtualAddress() + 256);
	}
	...
	static_cast<TCacheEntry*>(entry)->texture->TransitionToResourceState(D3D::currentCommandList, D3D12_RESOURCE_STATE_RENDER_TARGET);
	D3D::currentCommandList->OMSetRenderTargets(1, &static_cast<TCacheEntry*>(entry)->texture->GetRTV12(), FALSE, nullptr);
...
TextureCache::~TextureCache()
{
	for (unsigned int k = 0; k < MAX_COPY_BUFFERS; ++k)
	{
		if (efbcopycbuf12[k])
		{
			D3D::commandListMgr->DestroyResourceAfterCurrentCommandListExecuted(efbcopycbuf12[k]);
	...
	if (pTCacheEntrySaveReadbackBuffer)
	{
		D3D::commandListMgr->DestroyResourceAfterCurrentCommandListExecuted(pTCacheEntrySaveReadbackBuffer);
	}

	D3D::commandListMgr->DestroyResourceAfterCurrentCommandListExecuted(palette_buf12);
	D3D::commandListMgr->DestroyResourceAfterCurrentCommandListExecuted(palette_uniform12);
}
