...
void VertexManager::SetIndexBuffer()
{
	...
	D3D::currentCommandList->IASetIndexBuffer(&ibView);
}
...
{
	D3D::commandListMgr->DestroyResourceAfterCurrentCommandListExecuted(m_vertexBuffer);
	D3D::commandListMgr->DestroyResourceAfterCurrentCommandListExecuted(m_indexBuffer);
...
void VertexManager::Draw(u32 stride)
{
	...
	if (D3D::commandListMgr->dirtyVertexBuffer || oldStride != stride)
	{
		...
		D3D::commandListMgr->dirtyVertexBuffer = false;
		oldStride = stride;
	}
	...
	if (D3D::commandListMgr->currentTopology != d3dPrimitiveTopology)
	{
		D3D::currentCommandList->IASetPrimitiveTopology(d3dPrimitiveTopology);
		D3D::commandListMgr->currentTopology = d3dPrimitiveTopology;
	}

	D3D::currentCommandList->DrawIndexedInstanced(indices, 1, startIndex, baseVertex, 0);
...
void VertexManager::vFlush(bool useDstAlpha)
{
	...
	D3D::commandListMgr->_drawsSinceLastExecution++;
	...
	if (D3D::commandListMgr->_drawsSinceLastExecution > 100 && D3D::commandListMgr->_cpuAccessLastFrame)
	{
		D3D::commandListMgr->_drawsSinceLastExecution = 0;

		D3D::commandListMgr->ExecuteQueuedWork();
