...
static unsigned int    s_bbox_upload_heap_current_offset = 0;

static const unsigned int BBOX_UPLOAD_HEAP_SIZE = 2 * 1024 * 1024;
...
void BBox::Init()
{
	if (g_ActiveConfig.backend_info.bSupportsBBox)
	{
		...
		CheckHR(s_bbox_readback_buffer->Map(0, nullptr, &s_bbox_readback_buffer_data));
		...
		CheckHR(
			D3D::device12->CreateCommittedResource(
				...
				D3D12_HEAP_FLAG_NONE,
				&CD3DX12_RESOURCE_DESC::Buffer(BBOX_UPLOAD_HEAP_SIZE),
		...
		// Populate bounding box with initial values.
		BBox::Set(0, 0);
		BBox::Set(1, 0);
		BBox::Set(2, 0);
		BBox::Set(3, 0);
...
void BBox::Set(int index, int value)
{
	...
	s_bbox_upload_heap_current_offset = (s_bbox_upload_heap_current_offset + upload_size) % BBOX_UPLOAD_HEAP_SIZE;
}
...
int BBox::Get(int index)
{
	...
	return static_cast<s32*>(s_bbox_readback_buffer_data)[index];
}
